...
interface Condition extends Predicate<Context> {
    ...
    default boolean test(Context context) {
        ...
        if (isMet) {
            if (preferences) {
                log().info(context.currentKey(),
                        "will send " + getClass().getSimpleName() + "Notification because build matches and user preferences allow it");
            } else {
                log().debug(context.currentKey(),
                        "will NOT send " + getClass().getSimpleName() + "Notification - build matches but user preferences do not allow it");
            }
        } else {
